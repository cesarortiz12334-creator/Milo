...
// Íconos SVG de marca (paths estilo simple-icons, viewBox 0 0 24 24).
const REDES: { label: string; href: string; path: string }[] = [
  {
    label: "Instagram",
    href: "#",
    ...
    <footer className="border-t border-black/5 bg-white/60">
      <div className="mx-auto max-w-6xl px-4 py-10">
            ...
            <div className="mt-2 flex gap-2.5">
              {REDES.map((r) => (
                <a
                  ...
                  aria-label={r.label}
                  title={r.label}
                  className="flex h-9 w-9 items-center justify-center rounded-full bg-dark/5 text-dark/70 transition hover:bg-primary hover:text-white"
                >
                  <svg
                    viewBox="0 0 24 24"
        ...
        <p className="mt-8 border-t border-black/5 pt-6 text-center text-sm text-muted">
          MiloFund © 2026 — Hecho en Chile con amor 🐾
        </p>
      </div>
    </footer>
  );
